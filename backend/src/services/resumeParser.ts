...
export const parseResume = async (fileBuffer: Buffer): Promise<ParsedResumeData> => {
  try {
    ...
    const designationKeywords = ['software engineer', 'developer', 'manager', 'analyst', 'consultant', 
                                  'designer', 'architect', 'lead', 'senior', 'junior', 'associate'];
    for (const keyword of designationKeywords) {
      const regex = new RegExp(`\\b${keyword}\\b`, 'i');
      if (regex.test(text)) {
        const match = text.match(new RegExp(`((?:senior|junior|lead)?\\s*${keyword}(?:\\s+\\w+)?)`, 'i'));
        if (match) {
    ...
    const departmentKeywords = ['engineering', 'development', 'marketing', 'sales', 'hr', 'human resources',
